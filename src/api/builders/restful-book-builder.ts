...
class BookBuilder {
    ...
    private doCheck() {
        return this.booking.firstname !== undefined &&
    ...
    setBookingDates(checkinDate: string, checkoutDate: string) {
        if (this.isValidDateFormat(checkinDate) && this.isValidDateFormat(checkoutDate)) {
            ...
            throw new Error(`checkin: ${checkinDate}, checkout: ${checkoutDate}. Valid date format is YYYY-MM-DD`)
        }
    }
    ...
    build() {
        ...
        throw new Error(`The entity is not valid, fill this field: ${[this.findUndefinedFields(this.booking)]}`)
    }
}
...
function createRandomBooking () {
    return new BookBuilder()
        ...
        .setAdditionalNeeds(faker.helpers.arrayElement([
            'Breakfast', 'Extra pillow', 'Airport shuttle', 'Late checkout'
        ]))
        .build()
}
